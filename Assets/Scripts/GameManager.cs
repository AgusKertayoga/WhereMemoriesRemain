...
public class GameManager : MonoBehaviour
{
    ...
    [Range(0.5f, 5f)]
    [SerializeField] private float puzzleScale = 3f;
        ...
        for (int row = 0; row < dimensions.y; row++)
        {
            for (int col = 0; col < dimensions.x; col++)
            {
                ...
                piece.localPosition = new Vector3(
                  ((-width * dimensions.x / 2) + (width * col) + (width / 2)) * puzzleScale,
                  ((-height * dimensions.y / 2) + (height * row) + (height / 2)) * puzzleScale,
                  -1);
                piece.localScale = new Vector3(width * puzzleScale, height * puzzleScale, 1f);
    ...
    {
        // Jarak maksimum dari posisi asli
        float scatterDistance = 2f;

        for (int i = 0; i < pieces.Count; i++)
        {
            Transform piece = pieces[i];

            // Cari posisi asli puzzle
            ...
            // Acak sedikit di sekitar posisi asli
            float randomX = Random.Range(-scatterDistance, scatterDistance);
            float randomY = Random.Range(-scatterDistance, scatterDistance);

            piece.localPosition = originalPos + new Vector3(randomX, randomY, 0);
        }
    }
    ...
    private void UpdateBorder()
    {
        ...
        {
            Debug.LogWarning("LineRenderer tidak ditemukan pada GameHolder");
            return;
        }

        float halfWidth = (width * dimensions.x * puzzleScale) / 2f;
        float halfHeight = (height * dimensions.y * puzzleScale) / 2f;
    ...
    private void SnapAndDisableIfCorrect()
    {
        ...
        Vector2 targetPosition = new(((-width * dimensions.x / 2) + (width * col) + (width / 2)) * puzzleScale,
                                     ((-height * dimensions.y / 2) + (height * row) + (height / 2)) * puzzleScale);

        // Check if we're in the correct location.
        if (Vector2.Distance(draggingPiece.localPosition, targetPosition) < ((width * puzzleScale) / 2f))
        {
